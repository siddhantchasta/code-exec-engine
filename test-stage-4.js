...
// ---------------------------------------------------------------------------
// Stage 4 Gate Test — Full REST cycle
//
    ...
    const req = http.request(opts, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ status: res.statusCode || 0, body: /** @type {Record<string, unknown>} */ ({ raw: data }) });
        }
      });
    });
    req.on('error', reject);
...
async function runGateTests() {
  ...
  const submitRes = await request(
    ...
    '/submit',
    { 'x-api-key': API_KEY },
    JSON.stringify({ code: 'print("hello from gate test")', language: 'python' }),
  );
  ...
  if (!submissionId || !statusUrl) {
    console.error('❌ Gate 1 FAIL: missing id or statusUrl in response');
    process.exit(1);
  }

  console.log(`✅ Gate 1 PASSED: 202 Accepted — id=${submissionId}`);
  passed++;
  ...
  if (result.stdout !== 'hello from gate test\n') {
    errors.push(`stdout: got "${result.stdout}", expected "hello from gate test\\n"`);
  }
  ...
  // ---------------------------------------------------------------
  // Gate 5: Rate limiter — burst returns 429
  // ---------------------------------------------------------------
  console.log('\n--- Gate 5: Rate limit (burst requests) ---');

  // We've already consumed tokens from gates 1–4 (submit + polls + result).
  // Fire rapid requests until we hit 429.
  let got429 = false;
  let successCount = 0;
  const burstSize = 15;
  ...
  if (!got429) {
    console.error(`❌ Gate 5 FAIL: no 429 after ${burstSize} requests (got ${successCount} successes)`);
    process.exit(1);
  }

  console.log(`✅ Gate 5 PASSED: 429 returned after ${successCount} successful requests`);
  passed++;

  // ---------------------------------------------------------------
  // Summary
  // ---------------------------------------------------------------
  console.log(`\n🎉 ALL ${passed}/${total} STAGE 4 GATE TESTS PASSED 🎉\n`);
  process.exit(0);
}

runGateTests().catch((err) => {
  console.error('❌ Stage 4 test crashed:', err);
  process.exit(1);
});
